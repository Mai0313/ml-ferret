    ...
    def __init__(self, data_path, image_path, args) -> None:
        ...
        for idx, i in enumerate(datas):
            ...
            for match in matches:
                ...
                raw_box_coor = [
                    int(coor_cur[0] * img_raw_w),
                    int(coor_cur[1] * img_raw_h),
                    int(coor_cur[2] * img_raw_w),
                    int(coor_cur[3] * img_raw_h),
                ]
                converted_box_coor = [
                    int(coor_cur[0] * VOCAB_IMAGE_W),
                    int(coor_cur[1] * VOCAB_IMAGE_H),
                    int(coor_cur[2] * VOCAB_IMAGE_W),
                    int(coor_cur[3] * VOCAB_IMAGE_H),
                ]
                if args.add_region_feature:
                    question = question.replace(
                        match,
                        f"[{converted_box_coor[0]}, {converted_box_coor[1]}, {converted_box_coor[2]}, {converted_box_coor[3]}] {DEFAULT_REGION_FEA_TOKEN}",
                    )
                    generated_mask = generate_mask_for_feature(
                        raw_box_coor, raw_w=img_raw_w, raw_h=img_raw_h, mask=None
                    )
                    masks.append(generated_mask)
                else:
                    question = question.replace(
                        match,
                        f"[{converted_box_coor[0]}, {converted_box_coor[1]}, {converted_box_coor[2]}, {converted_box_coor[3]}]",
                    )
...
def eval_model(args):
    ...
    for i, id in enumerate(tqdm(data_ids)):
        ...
        ans_file.write(
            json.dumps({
                "question_id": ann["question_id"],  # +1 offset
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data_path", type=str, default="dataset/lvis/lvis_v1_minival_inserted_image_name.json"
    )
    ...
    parser.add_argument(
        "--region_format", type=str, default="box", choices=["point", "box", "segment"]
    )
