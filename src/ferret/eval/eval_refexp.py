...
class RefExpEvaluatorFromJsonl(object):
    ...
    def summarize(self, prediction_file: str, verbose: bool = False):
        ...
        if os.path.isfile(prediction_file):
            ...
            predictions = [
                json.loads(line)
                for pred_file in os.listdir(prediction_file)
        ...
        for item_img, item_ann in tqdm(
            zip(self.refexp_gt["images"], self.refexp_gt["annotations"])
        ):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--annotation_file", default="/path/to/json_annotations", help="annotation_file"
    )

    args = parser.parse_args()

    evaluator = RefExpEvaluatorFromJsonl(
        refexp_gt_path=args.annotation_file, k=(1, "mean", "upper bound"), thresh_iou=0.5
    )
