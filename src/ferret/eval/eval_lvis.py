...
def get_args():
    ...
    parser.add_argument(
        "--pred_file",
        type=str,
        default="/Users/youhaoxuan/research_misc/lvis_result/llava_answer_debug.jsonl",
    )
...
if __name__ == "__main__":
    ...
    if os.path.isfile(args.pred_file):
        ...
        predictions = [
            json.loads(line)
            for pred_file in os.listdir(args.pred_file)
            for line in open(os.path.join(args.pred_file, pred_file))
        ]
    else:
