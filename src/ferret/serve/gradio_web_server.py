...
from ferret.utils import build_logger, server_error_msg, violates_moderation, moderation_msg
import hashlib
...
def draw_box(coor, region_mask, region_ph, img, input_mode):
    ...
    if input_mode == "Box":
        ...
        draw.text(
            [coor[0] + int(font.size * 0.2), coor[1]], region_ph, font=font, fill=(255, 255, 255)
        )
...
def format_region_prompt(prompt, refer_input_state):
    ...
    refer_input_state["region_masks_in_prompts"] = [
        refer_input_state["region_masks"][iii] for iii in indices_region_placeholder_in_prompt
    ]
    ...
    for region_ph_index, region_ph_i in enumerate(refer_input_state["region_placeholder_tokens"]):
        prompt = prompt.replace(
            region_ph_i,
            "{} {}".format(
                refer_input_state["region_coordinates"][region_ph_index], DEFAULT_REGION_FEA_TOKEN
            ),
        )
    return prompt
...
def draw(input_mode, input, refer_input_state, refer_text_show, imagebox_refer):
    ...
    mask_new = (
        F.interpolate(mask_new.unsqueeze(0).unsqueeze(0), (img_height, img_width), mode="bilinear")
        > 0
    )
...
def build_demo(embed_mode):
    ...
    with gr.Blocks(title="FERRET", theme=gr.themes.Base(), css=css) as demo:
        ...
        with gr.Row():
            with gr.Column(scale=4):
                ...
                refer_input_mode = gr.Radio(
                    ["Point", "Box", "Sketch"], value="Point", label="Referring Input Type"
                )
            ...
            with gr.Column(scale=5):
                chatbot = gr.Chatbot(elem_id="chatbot", label="FERRET", visible=False).style(
                    height=750
                )
                with gr.Row():
        ...
        regenerate_btn.click(
            regenerate, [state, image_process_mode], [state, chatbot, textbox] + btn_list
        ).then(
            http_bot,
        ...
        location_btn.click(
            show_location, [sketch_pad, chatbot], [imagebox_output, chatbot, location_btn]
        )
        ...
        if args.model_list_mode == "once":
            demo.load(
                load_demo,
                [url_params],
                [state, model_selector, chatbot, textbox, submit_btn, button_row, parameter_row],
