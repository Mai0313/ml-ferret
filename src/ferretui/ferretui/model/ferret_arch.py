    ...
    def extract_region_feature(
        ...
        for region_feature_map_i, region_masks_list_i in zip(region_feature_map, region_masks):
            if len(region_masks_list_i) == 0:
                ...
                # list of elements of shape [num_sample_point, 2]
                non_zero_pos = [
                    rand_sample((m.nonzero() / ori_image_wh), self.get_model().max_sample_point)
                    for m in region_masks_list_i
                ]
                ...
                region_feature_i = torch.stack([
                    x[m].mean(dim=0)
                    for x, m in zip(region_feature_i.transpose(1, 2), non_zero_pos_mask)
                ]).nan_to_num()
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if type(images) is list or images.ndim == 5:
            ...
            raw_image_features, image_features, region_feature_map = self.encode_images(
                concat_images, region_flag=region_flag, region_geo_sampler=region_geo_sampler
            )
            split_sizes = [image.shape[0] for image in images]
            image_features = torch.split(image_features, split_sizes, dim=0)

            if region_flag:
                region_feature_maps = torch.split(
                    region_feature_map, split_sizes, dim=0
                )  #  (#images, #patches, h*w, c)
            ...
            if mm_patch_merge_type == "flat":
                ...
                first_region_feature_map = [x[0:1] for x in region_feature_map]
                region_feature_map = torch.cat(
                    first_region_feature_map, dim=0
                )  #  (#images, h, w, c)
                ...
                for image_idx, image_feature in enumerate(image_features):
                    if image_feature.shape[0] > 1:
                        ...
                        if region_flag:
                            cur_region_feature_map = region_feature_maps[
                                image_idx
                            ]  # (#patches, h*w, c)
                            cur_region_feature_map = cur_region_feature_map.view(
                                cur_region_feature_map.shape[0],
                                height,
                                width,
                                cur_region_feature_map.shape[-1],
                            )  # (#patches, h, w, c)
                        ...
                        if image_aspect_ratio == "anyres":
                            ...
                            if region_flag:
                                region_feature = region_feature.view(
                                    num_patch_height, num_patch_width, height, width, -1
                                )
                        else:
                        ...
                        if region_flag:
                            region_feature = (
                                region_feature.permute(0, 2, 1, 3, 4).contiguous()
                            )  # (patch_h, patch_w, h, w, c) -> (patch_h, h, patch_w, w, c)
                            region_feature = region_feature.flatten(0, 1).flatten(
                                1, 2
                            )  # (patch_h, h, patch_w, w, c) -> (all_h, all_w, c)
                            ...
                            base_region_feature_resized = F.interpolate(
                                base_region_feature.unsqueeze(0).permute(0, 3, 1, 2),
                                (region_feature.shape[0], region_feature.shape[1]),
                            )  # (1, c, all_h, all_w)
                            base_region_feature_resized = base_region_feature_resized.to(
                                region_feature.dtype
                            )
                            base_region_feature_resized = base_region_feature_resized.squeeze(
                                0
                            ).permute(1, 2, 0)  # (all_h, all_w, c)
                            # === Add:
        ...
        if region_flag:
            ...
            for img_idx, (cur_input_id, cur_region_mask) in enumerate(
                zip(input_ids, region_masks)
            ):
