...
class CLIPImageProcessor_Ferret(CLIPImageProcessor):
    def resize(
        ...
        **kwargs,
        ...
        return resize(
            image, size=output_size, resample=resample, data_format=data_format, **kwargs
        )
...
class CLIPVisionTower(nn.Module):
    ...
    def load_model(self, device_map=None):
        ...
        if "ferret" in self.preprocess_type:
            self.image_processor = CLIPImageProcessor_Ferret.from_pretrained(
                self.vision_tower_name
            )
        else:
...
class CLIPVisionTowerS2(CLIPVisionTower):
    def __init__(self, vision_tower, args, delay_load=False):
        ...
        if not delay_load or getattr(args, "unfreeze_mm_vision_tower", False):
            ...
            self.image_processor.crop_size["height"] = self.image_processor.crop_size["width"] = (
                self.s2_image_size
            )
