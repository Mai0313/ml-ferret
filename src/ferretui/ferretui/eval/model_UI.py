...
class UIData:
    ...
    def __getitem__(self, idx):
        ...
        if self.task == "box_in":
            ...
            box_x1_textvocab, box_y1_textvocab, box_x2_textvocab, box_y2_textvocab = get_bbox_coor(
                box=box, ratio_h=ratio_h, ratio_w=ratio_w
            )

            if self.args.region_format == "box":
                region_coordinate_raw = [box_x1, box_y1, box_x2, box_y2]
                if args.add_region_feature:
                    i["question"] = prompt.replace(
                        "<bbox_location0>",
                        "[{}, {}, {}, {}] {}".format(
                            int(box_x1_textvocab),
                            int(box_y1_textvocab),
                            int(box_x2_textvocab),
                            int(box_y2_textvocab),
                            DEFAULT_REGION_FEA_TOKEN,
                        ),
                    )
                    generated_mask = generate_mask_for_feature(
                        region_coordinate_raw, raw_w=i["image_w"], raw_h=i["image_h"], mask=None
                    )
...
def eval_model(args):
    ...
    tokenizer, model, image_processor, context_len = load_pretrained_model(
        model_path, args.model_base, model_name, use_safetensors=True
    )
    ...
    for i, id in enumerate(tqdm(chunk_data_ids)):
        ...
        if model.config.image_aspect_ratio == "square_nocrop":
            ...
            image_tensor = process_images(
                [img], image_processor, model.config, image_process_func=image_process_func
            )[0]
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="point",
        choices=["point", "box", "segment", "free_shape"],
    )
