...
class DataArguments:
    ...
    data_multiple: List[float] = field(
        default=None,
        metadata={
            "help": "Data mutliplier for each dataset when mixed. None means direct concat."
        },
    )
...
class LazySupervisedDataset(Dataset):
    ...
    def shard_data(self, datas, data_path, ori_counts):
        ...
        datas = datas[
            no_shard_per_worker * self.global_rank : no_shard_per_worker * (self.global_rank + 1)
        ]
    ...
    def __init__(
        ...
        for data_path_i, image_folder_i in zip(data_path, image_folders):
            if "blip_laion_cc_sbu" in data_path_i:
                ...
                list_data_dict.append(
                    self.load_conversation_perception(data_path_i, image_folder_i)
                )
            elif "conversation_interaction" in data_path_i:
                logging.warning(f"Loading conversation_interaction data")
                list_data_dict.append(
                    self.load_conversation_interaction(data_path_i, image_folder_i)
                )
    ...
    def sync_iter_counts(self):
        ...
        print(
            f"[R{self.global_rank}] sync_iter_counts at {os.path.basename(__file__)}: ori dataset counts: {num_samples}, now mmx list dataset len: {len(self.list_data_dict)}."
        )
    ...
    def generate_mask_for_feature(self, coor, box, mask, raw_w, raw_h):
        ...
        if len(coor) == 2:
            ...
            assert (coor_mask == 1).any(), (
                f"coor: {coor}, box: {box}, raw_w: {raw_w}, raw_h: {raw_h}"
            )
        elif len(coor) == 4:
    ...
    def format_unicode_filenames(filename):
        ...
        return re.subn(r"(#U[0-9a-f]{4})", lambda cp: chr(int(cp.groups()[0][2:], 16)), filename)[
            0
        ]
                    ...
                    for box_idx, box_i in enumerate(box_list_i):
                        ...
                        if location_format == "box":
                            ...
                            if "point_box_cache" in locals() and tuple(box_i) in point_box_cache:
                                ...
                                raw_coor_i = self.get_bbox_coor(
                                    box=box_i, ratio_w=ratio_w, ratio_h=ratio_h
                                )
                            ...
                            if "point_box_cache" in locals() and tuple(box_i) in point_box_cache:
                                ...
                                if "segment_mask" in self.point_input_sample:
                                    if "masks" in sources[0]:
                                        cur_mask = deepcopy(
                                            sources[0]["masks"][box_list_idx][box_idx]
                                        )
                                        ...
                                        if "uniform" in self.point_input_sample.split("-")[1]:
                                            obj_center_x, obj_center_y = (
                                                self.sample_point_in_segment(
                                                    mask=cur_mask, ratio_w=ratio_w, ratio_h=ratio_h
                                                )
                                            )
                                        ...
                                        obj_center_x, obj_center_y = self.get_obj_center(
                                            box=box_i,
                                            ratio_w=ratio_w,
                                            ratio_h=ratio_h,
                                            std_dev_weight=0.15,
                                    ...
                                    raise NotImplementedError(
                                        f"Not support {self.point_input_sample} in data sampling"
                                    )
                        ...
                        assert f"<bbox_location{box_idx}>" in cur_conv, (
                            f"String '<bbox_location{box_idx}>' not found in {cur_conv}"
                        )
                        if self.add_region_feature and box_list_idx % 2 == 0:
                            if self.no_coor:
                                cur_conv = cur_conv.replace(
                                    f"<bbox_location{box_idx}>", f"{DEFAULT_REGION_FEA_TOKEN}"
                                )
                            else:
                                cur_conv = cur_conv.replace(
                                    f"<bbox_location{box_idx}>",
                                    coor_i + f" {DEFAULT_REGION_FEA_TOKEN}",
                                )
                            cur_box = box_i
                            cur_mask = (
                                deepcopy(sources[0]["masks"][box_list_idx][box_idx])
                                if "masks" in sources[0]
                                else None
                            )
                            ori_size_raw_coor_i = (
                            ...
                            cur_region_mask = self.generate_mask_for_feature(
                                ...
                                cur_box,
                                cur_mask,
                                raw_w=sources[0]["image_w"],
                                raw_h=sources[0]["image_h"],
                            )
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        ...
        if training_args.unfreeze_mm_vision_tower:
            lr_of_vit = (
                training_args.mm_vision_tower_lr
        ...
        model.initialize_vision_tokenizer(
            model_args, tokenizer=tokenizer, add_region_feature=model_args.add_region_feature
        )
