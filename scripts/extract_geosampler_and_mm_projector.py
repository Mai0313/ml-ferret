...
def parse_args():
    ...
    parser.add_argument(
        "--keys_to_match",
        type=str,
        default="region_geo_sampler",
        choices=["mm_projector", "region_geo_sampler"],
        help="keys to be matched",
    )
...
if __name__ == "__main__":
    ...
    try:
        ...
        v = "pytorch_model.bin"
        for k in torch.load(os.path.join(args.model_path, v), map_location="cpu"):
            if any(key_match in k for key_match in keys_to_match):
                ckpt_to_key[v].append(k)
